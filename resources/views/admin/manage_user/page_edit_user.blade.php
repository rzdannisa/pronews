                ...
                <tbody>
                  ...
                  @foreach($users->typeuser as $type)
                    <tr>
                      ...
                      <td>
                        <a href="{{ url('manage_user/edit_user/'.$users->id)}}"><button style="width:90px;" type="button" class="btn btn-info">Edit</button></a>
                      </td>
                      <td>
                        <form style="display: inline-table;" method="POST" action="{{ url('manage_user/delete_user') }}">
                          <input type="hidden" name="id" value="{{$users->id}}">
                          <input type="hidden" name="_token" value="{{ csrf_token() }}">
                          <button style="width:90px;" onclick="return confirm('Are you sure you want to delete this users ?')" class="btn btn-danger" type="submit">Delete</button>
                        </form>
                      </td>
                    </tr>
                  @endforeach
